...
use scuffle_config::{Config, ConfigBuilder, Key, KeyGraph, KeyPath, Source, Value};
...
fn config_derive_cli() {
	#[derive(Debug, Default, PartialEq, serde::Deserialize, scuffle_config::Config)]
	#[serde(default)]
	struct AppConfig {
		value: String,
		enabled: bool,
...
fn config_derive_nested_cli() {
	...
	#[derive(Debug, PartialEq, serde::Deserialize, scuffle_config::Config)]
	#[serde(default)]
	struct SubConfig {
	...
	let duration = scuffle_config::parse_key::<std::time::Duration>(value).unwrap();
	assert_eq!(
		duration,
	...
	let time = scuffle_config::parse_key::<std::time::SystemTime>(value).unwrap();
	assert_eq!(
		time,
...
fn config_derive_cyclic() {
	...
	let config: AppConfig = scuffle_config::parse_key(value).unwrap();
...
fn cli_vector_derive() {
	...
	let count = scuffle_config::parse_key::<Vec<u32>>(value).unwrap();
...
fn fringe_types() {
	...
	// by the scuffle_config::Config trait since it has a validator function.
...
fn flattened_config() {
	#[derive(Debug, Default, PartialEq, serde::Deserialize, scuffle_config::Config)]
	#[serde(default)]
	struct AppConfig<F: scuffle_config::Config> {
		#[config(flatten)]
		#[serde(flatten)]
		flattened: F,
	}
