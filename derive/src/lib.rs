...
fn impl_config(ast: &syn::DeriveInput) -> syn::Result<TokenStream> {
	...
	for field in fields.named.iter() {
		...
		let type_attr = attributes
			...
			.map(|t| quote! { #t })
			.unwrap_or_else(|| quote! { <#path as ::scuffle_config::Config>::graph() });

		keys_init.push(if flatten {
			quote! {
				{
					let mut graph = #type_attr;
					while let ::scuffle_config::KeyGraph::Option(g) = graph.as_ref() {
						graph = g.clone();
					}

					match graph.as_ref() {
						::scuffle_config::KeyGraph::Struct(flat_keys) => {
							keys.extend(flat_keys.clone());
						}
						::scuffle_config::KeyGraph::Unit => {},
						_ => {
			...
			let transform_attr = attributes
				...
				.find_map(|a| if let Attr::KeyType(e) = a { Some(e) } else { None })
				.map(|e| {
					quote! { |path: &::scuffle_config::KeyPath, value: ::scuffle_config::Value| {
						::scuffle_config::transform_from_graph(path, &#e, value)
					}}
				})
				.unwrap_or_else(|| quote! { <#path as ::scuffle_config::Config>::transform });
			...
			let add_attrs = {
				...
				quote! {
					let key = ::scuffle_config::Key::new(#type_attr);
					#env_attr
					#cli_attr
	...
	let name = &ast.ident;
	Ok(quote! {
		impl #impl_generics ::scuffle_config::Config for #name #ty_generics #where_clause {
			...
			fn graph() -> ::std::sync::Arc<::scuffle_config::KeyGraph> {
				let builder = ::scuffle_config::KeyGraph::builder::<Self>();
				...
				builder.build(::scuffle_config::KeyGraph::Struct(keys))
			}
		}
	}
	.into())
